...
// ====================================================================================
// === 親(QP)へ postMessage するためのユーティリティ ===
const PARENT_ORIGIN = "https://uniofbath.questionpro.eu";  // ← 実際のドメインに合わせて
function postToParent(message) {
  try { window.parent?.postMessage?.(message, PARENT_ORIGIN); } catch (_) {}
}

// （任意）高さ連携：iframeの見切れ防止
const postHeight = () => {
  const h = Math.max(
    document.body.scrollHeight, document.documentElement.scrollHeight,
    document.body.offsetHeight,  document.documentElement.offsetHeight
  );
  postToParent({ type: "jspsych:resize", height: h });
};
...
const jsPsych = initJsPsych({
  on_finish: () => {
    ...
    // ① 親(QP)へ「実験完了＋データ」を通知（これが“Next解放”のトリガーになる）
    postToParent({ type: 'jspsych:final', payload });

    // ②（任意）参加者の手元にもJSONをダウンロード
